...
export const TakoIcon = ({ id, pun, index }: TakoIconProps): JSX.Element => {
  ...
  const imgurl = useRandom
    ? `${process.env.PUBLIC_URL}/icon/${images[prng(index) % images.length]}`
    : `${process.env.PUBLIC_URL}/takos/${id}`;
  ...
  return (
    <TakoImgContainer onClick={!pun ? undefined : togglePun}>
      ...
      <TakoImg alt={`tako-icon-${id}`} src={imgurl} />
      {punVisible && (
        <PunContainer onClick={(e) => e.stopPropagation()}>
          <PunDisplay>{pun}</PunDisplay>
        </PunContainer>
      )}
    </TakoImgContainer>
  );
